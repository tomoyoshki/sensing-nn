...
class WindNoiseGenerator(object):
    def __init__(self, mat_file):
        """
        Reference: Nelke, C.M. and Vary, P., 2014, September. Measurement, analysis and simulation of wind noise signals
        for mobile communication devices. In 2014 14th International Workshop on Acoustic Signal Enhancement (IWAENC).
        Source code: https://github.com/cohendrake/WindNoiseGenerator/blob/main/generate_wind_noise.py
        """
    ...
    def generate(self, duration, fs=16000, type="const"):
        """
        duration: duration of the wind noise in seconds
        fs: sampling frequency
        type: "gusts" or "const"
        """
        L = int(fs * duration)
        excite_sig_noise = np.random.randn(L) * np.sqrt(self.var_excitation_noise)
        ...
        for k in range(self.lpc_order, L):
            ...
            n[k] = np.sum(
                (
                    g_apl[k] * exc_sig
                    + np.flip(n[k - self.lpc_order : k], axis=0)
                    + g_apl_LT[k] * excite_sig_noise[k] * self.var_excitation_noise
                )
                * self.lpc_coeff
            )
...
def write_wind_noise(wind_generator, idx, fs, duration, wind_type, out_dir):
    ...
    out_file = os.path.join(out_dir, "{:s}_{:06d}.wav".format(wind_type, idx))
...
if __name__ == "__main__":
    ...
    wind_generator = WindNoiseGenerator(mat_file)
    num_worker = 20
    ...
    for i in range(numClip):
        res = pools.apply_async(write_wind_noise, args=(wind_generator, i, fs, duration, wind_type, out_dir))
        res_list.append(res)
