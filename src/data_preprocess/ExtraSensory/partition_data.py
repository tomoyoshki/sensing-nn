...
class_probs = {1: 81081, 3: 24713, 0: 65511, 2: 10777}
...
def partition_data(paired_data_path, output_path, user_list, train_ratio=0.8):
    ...
    train_user_count = int(len(user_list) * train_ratio)
    val_user_count = round(len(user_list) * (1 - train_ratio) / 2)
    ...
    for sample_file in tqdm(data_samples):
        user = os.path.basename(sample_file).split("_")[0]
        file_path = os.path.join(os.path.join(paired_data_path, sample_file))
        ...
        modality_flags = load_sample["flag"]["phone"]
        complete_modality_flag = np.product(list(modality_flags.values())) > 0
        label = int(load_sample["label"].float().numpy())
...
if __name__ == "__main__":
    ...
    paired_data_path = f"/home/{username}/data/ExtraSensory/paired_data"
    output_path = f"/home/{username}/data/ExtraSensory/data_partition"

    # extract user list
    label_path = f"/home/{username}/data/ExtraSensory/clean_labels"
